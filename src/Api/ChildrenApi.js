...
export async function register(userObj){
    let clientId=userObj.surname[0]+userObj.email[3]+Date.now().toString()
...
export async function updateAddress(userObj){
    return sendRequest(
        `auth/client/address/update`,
        requestMethod.PUT,
        auth.TRUE,
        userObj,
    )
...
/***
 * CRUD for Prefer Location
 *
 * @param userObj
 * @returns {Promise<*>}
 */
export async function getPreferLocationInfo(){
    return sendRequest(
        `auth/prefer-location/entity/`,
        requestMethod.GET,
        auth.TRUE,
    )
}

export async function updatePreferLocation(userObj){
    return sendRequest(
        `auth/prefer-location/update/`,
        requestMethod.PUT,
        auth.TRUE,
        userObj,
    )
